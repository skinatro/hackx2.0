...
const navCubes: NavCube[] = [
  ...
  { label: "Problem Statement", shortLabel: "PS", href: "/ps", accent: "#5ce1e6" },
    ...
    <aside className="pointer-events-none fixed right-3 top-1/2 z-20 -translate-y-1/2 sm:right-5 xl:right-7">
      <nav className="pointer-events-auto flex flex-col gap-3" aria-label="Primary">
        {navCubes.map((item) => {
          ...
          return (
            <div key={item.href} className="group relative flex items-center justify-end">
              <div
                ...
                style={{ backgroundColor: item.accent, boxShadow: "4px 4px 0 #000" }}
              >
                {item.label}
              </div>
              <Link
                ...
                className={`flex h-14 w-14 items-center justify-center border-2 text-xs font-black uppercase tracking-[0.22em] backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 sm:h-15 sm:w-15 ${isActive
                ...
                style={isActive ? { boxShadow: `6px 6px 0 ${item.accent}` } : undefined}
              >
                <span className="navbar-font text-lg leading-none sm:text-xl">{item.shortLabel}</span>
              </Link>
            </div>
          );
        })}
